...
static void http_send_status_json(uint8_t s, int code, const char *json)
{
    ...
    if (json && *json) send(s, (uint8_t*)json, (uint16_t)strlen(json));
}
...
static void http_send_status_text(uint8_t s, int code, const char *txt)
{
    ...
    if (txt && *txt) send(s, (uint8_t*)txt, (uint16_t)strlen(txt));
}
...
static void json_get_s(cJSON* obj, const char* k, char* out, size_t outsz, const char* defv) {
    cJSON* j = cJSON_GetObjectItemCaseSensitive(obj, k);
    if (cJSON_IsString(j) && j->valuestring) strzcpy(out, outsz, j->valuestring);
    else strzcpy(out, outsz, defv?defv:"");
}
static int parse_ip4(const char* s, uint8_t ip[4]) {
    if (!s) return 0;
    int a,b,c,d;
    if (sscanf(s, "%d.%d.%d.%d",&a,&b,&c,&d)!=4) return 0;
    if ((unsigned)a>255||(unsigned)b>255||(unsigned)c>255||(unsigned)d>255) return 0;
    ip[0]=a; ip[1]=b; ip[2]=c; ip[3]=d;
    return 1;
}
static void add_ip_str(cJSON* o, const char* k, const uint8_t ip[4]) {
    char buf[16]; snprintf(buf,sizeof(buf),"%u.%u.%u.%u",ip[0],ip[1],ip[2],ip[3]);
...
static void api_network(uint8_t s, void *req_)
{
    ...
    if (req->METHOD == METHOD_POST) {
        ...
        if (!j) { http_send_status_json(s, 400, "{\"error\":\"invalid json\"}"); return; }

        // ip/mask/gw/dns (string)
        cJSON* v;
        v = cJSON_GetObjectItemCaseSensitive(j,"ip");   if (cJSON_IsString(v)&&v->valuestring) parse_ip4(v->valuestring,cfg.ip);
        v = cJSON_GetObjectItemCaseSensitive(j,"mask"); if (cJSON_IsString(v)&&v->valuestring) parse_ip4(v->valuestring,cfg.sn);
        v = cJSON_GetObjectItemCaseSensitive(j,"gw");   if (cJSON_IsString(v)&&v->valuestring) parse_ip4(v->valuestring,cfg.gw);
        v = cJSON_GetObjectItemCaseSensitive(j,"dns");  if (cJSON_IsString(v)&&v->valuestring) parse_ip4(v->valuestring,cfg.dns);
...
static void api_config(uint8_t s, void *req_)
{
    ...
    if (req->METHOD == METHOD_POST)
    {
        ...
        if (cJSON_IsString(j_psu) && j_psu->valuestring) {
            ...
            if (v<=247) cfg.psu_slave_addr = (uint8_t)v;
        } else if (cJSON_IsNumber(j_psu)) {
            int v = j_psu->valueint; if (v>=1 && v<=247) cfg.psu_slave_addr = (uint8_t)v;
        }
        ...
        if (cJSON_IsString(j_ip_snmp) && j_ip_snmp->valuestring) parse_ip4(j_ip_snmp->valuestring, cfg.snmp_manager_ip);
        ...
        if (cJSON_IsObject(p)) {
            ...
            if (cJSON_IsNumber(jt)) {
                int t=jt->valueint; if (t==12||t==24||t==36||t==48) cfg.psu.battery_type=(battery_type_t)t;
            } else if (cJSON_IsString(jt) && jt->valuestring) {
                const char* s = jt->valuestring;
                if (!strcmp(s,"12")||!strcmp(s,"12V")) cfg.psu.battery_type=12;
                else if (!strcmp(s,"24")||!strcmp(s,"24V")) cfg.psu.battery_type=24;
                else if (!strcmp(s,"36")||!strcmp(s,"36V")) cfg.psu.battery_type=36;
                else if (!strcmp(s,"48")||!strcmp(s,"48V")) cfg.psu.battery_type=48;
            }
            ...
            if (cJSON_IsNumber(jp) && jp->valueint>0 && jp->valueint<65536) cfg.psu.snmp_trap_port = (uint16_t)jp->valueint;
        ...
        if (!appcfg_validate(&cfg, why, sizeof(why))) {
            cJSON_Delete(j);
            char buf[160]; snprintf(buf,sizeof(buf),"{\"error\":\"%s\"}", why);
    ...
    if (req->METHOD == METHOD_GET)
    {
        ...
        char psu_hex[8]; snprintf(psu_hex,sizeof(psu_hex),"%02X", cfg.psu_slave_addr);
