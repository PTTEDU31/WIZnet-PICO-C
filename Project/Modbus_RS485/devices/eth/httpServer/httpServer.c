...
// Private Functions
////////////////////////////////////////////
void send_http_response_header(uint8_t s, uint8_t content_type, uint32_t body_len, uint16_t http_status)
{
	switch (http_status)
	{
		...
		if (strncmp((char *)uri_name, "api/", 4) == 0)
		{
			for (uint8_t i = 0; i < api_count; i++)
			{
				if (!strcmp((char *)uri_name, api_table[i].path))
				{
					...
					return; // ✅ tránh xử lý file tĩnh
				}
			}
			// Không tìm thấy API phù hợp
			const char *err = "{\"error\":\"API not found\"}";
			send_http_response_header(s, PTYPE_JSON, strlen(err), STATUS_NOT_FOUND);
